...
<!--
  ...
  | Infrastructure/Security Audits | `[ORG]/OPS/Infrastructure/` | `YYYYMMDD-[AuditType]-Audit.docx` |
...
- [ ] **For any hand-authored `.md` or `.py` file (context files, session logs, plan and issue bodies, scripts, docstrings):** Run a banned-character grep before saving, then a quick banned-vocabulary pass. Fix every hit and re-scan clean before the file is written or committed. Having the banned-writing rules in context is not enough: em dashes keep slipping into headings, appositives, and string literals while drafting. This applies to ad-hoc authoring, not just build-script output. Reusable check:

  ```bash
  # em dash, en dash, double hyphen (eyeball-exclude markdown table rules / HTML comments)
  grep -nP '\x{2014}|\x{2013}|--' FILE
  # quick vocabulary pass against your own ban list
  grep -nioE '\b(crucial|pivotal|vital|robust|seamless|leverage|showcase|enhance|underscore|delve)\b' FILE
  ```
