...
- [ ] **For any document generated by a build script or tool (docx, pptx, pdf):** Extract full text before delivery (mammoth for docx, pdfminer for pdf), run it against your banned-style vocabulary list, and confirm zero hits. Do not deliver the file link until the scan returns clean. Build scripts can embed violations in string literals written in a prior session — passive self-review of generated output is not sufficient.
...
Full rules live in your SEO style guide. Core rules:

- Identify search intent before writing: informational, commercial, or transactional.
- One primary keyword, 2-5 secondary. Primary keyword in: title, H1, first 100 words, one H2, meta description.
- Title tag: 50-60 characters. Meta description: 140-160 characters with a CTA.
- Keyword density: 1-2%, never more. No exact-match forcing — use natural variations.
- Every image has descriptive alt text. Every page has internal links.

---

## 8. AI Agent Working Style Best Practices

Read your top-level instruction file (CLAUDE.md or equivalent) directly. Do not rely on summaries here.

---

## 9. Document Quality Checklist
...
- [ ] **For any document generated by a build script or tool (docx, pptx, pdf):** Extract full text before delivery, run it against the banned-style vocabulary list, and confirm zero hits. Do not deliver the file link until the scan returns clean. Passive checklist review of your own output is not sufficient — the script may embed violations in string literals that were written in a prior session.
- [ ] No hollow openers, no rule-of-three reflexes, no uniform sentence lengths
- [ ] All diagrams rendered to PNG/SVG before embedding — not raw Mermaid code
- [ ] All credentials loaded from a secrets store, never hardcoded

---

*Read this file at the start of any documentation task.*
*Last updated: 2026-03-31 — Added Sections 7 (SEO), 8 (AI Working Style), 9 (Document Quality Checklist including mandatory pre-delivery banned-style scan for build-generated documents).*
...
| Date | What Failed | Root Cause | Fix Applied | ERRORS.md Ref |
|------|-------------|------------|-------------|---------------|
| 2026-03-28 | Build script produced a docx with banned-style violations in string literals from a prior session. Checklist item was passive — no artifact scan existed. | Missing Gate: no post-build scan. Prior-session string literals bypass in-session writing rules. | Added mandatory pre-delivery scan to Section 9. | — |

**Notes:**
<!-- Per-entry context that doesn't fit in the table. -->
