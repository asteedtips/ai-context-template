...
- [ ] Update plan doc: mark items DONE, record commit hashes, note CI run number (if your CI uses `paths-ignore: docs/**` or equivalent, the docs-only plan commit will not trigger a CI run by design; do not expect a "CI green" gate on this commit)
