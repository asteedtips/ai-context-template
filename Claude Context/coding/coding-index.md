...
**`coding-smoke-testing.md`**: The agent-driven browser pass against a real deployed build, the only verification step in the graph that looks at the rendered page rather than at source. Deploy by dispatching the dev CD workflow the repo already has; there is deliberately no smoke-specific workflow file, and Section 5.3 records why one was specified, never built, and struck. Covers the deploy-and-hold lifecycle (dev keeps the feature build so a person tests what the agent verified, and no scheduled job may restore automatically), the deploy-and-merge authority the agent owns end to end, the fix-and-redeploy loop that runs until the checklist is clean, the dedicated smoke accounts plus the documented exception for a maintainer's own dev session, what the browser tooling has to be able to do (console and network capture per row is the non-negotiable one), the six traps that produce false readings when driving a server-rendered interactive page, and the evidence artifact format where every row carries observed behavior. **Read when:** any change reaches the smoke stage, when setting a new repo up for it, or before writing Section 8h checklist rows, since rows an agent can run are written differently from rows a human walks.
